...
const config = JSON.parse(fs.readFileSync(path.join(__dirname, "pogiko", "configs.json"), "utf8")); 
...
const reloadFiles = async (directory, type) => {
    console.log(
        `${type.charAt(0).toUpperCase() + type.slice(1)} directory changed. Reloading...`,
    );
    if (type === "commands") {
            ...
            api.listenMqtt(async (err, event) => {
                ...
                for (const [commandName, command] of global.modules.commands) {
                    if (
                        ...
                        command.autoActivate(event.body)
                    ) {
                        try {
                            await command.execute(api, event, []);
                        } catch (error) {
                            console.error(
                                `Auto command execution error: ${error}`,
                            );
                        }
                        return;
                    }
                ...
                const command =
                    global.modules.commands.get(commandName) ||
                    global.modules.commands.get(commandName.toLowerCase());
                ...
                if (command) {
                    if (isPrefixed && command.prefixRequired === false) {
                        ...
                        api.sendMessage(
                            global.convertToGothic(
                                "This command requires a prefix to start.",
                            ),
                            event.threadID,
                            event.messageID,
                        );
                        ...
                        command.adminOnly &&
                        !global.adminBot.includes(event.senderID)
                    ) {
                        api.sendMessage(
                            global.convertToGothic(
                                "Only bot admins have access to this command.",
                            ),
                            event.threadID,
                            event.messageID,
                        );
                    ...
                    api.sendMessage(
                        global.convertToGothic(
                            `The command "${commandName}" does not exist. Please type ${global.prefix}help to see the list of commands.`,
                        ),
                        event.threadID,
                        event.messageID,
                    );
